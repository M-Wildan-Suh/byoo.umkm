<div x-data="highlightManager({{ json_encode($product->productHighlight) }}, '{{ Auth::user()->role }}')" class="w-full grid lg:grid-cols-2 gap-4">
                ...
                <div class="rounded-xl flex justify-between gap-4 bg-white">
                    ...
                    <div class="flex flex-col flex-grow justify-between gap-2">
                        ...
                        <textarea x-model="form.description"
                            class="min-w-0 w-full p-0 border-t-0 border-l-0 border-r-0 ring-0 focus:ring-0 text-sm" placeholder="Deskripsi"
    ...
    <template x-if="canAddHighlight">
        <form x-data="highlightCreateForm()" @submit.prevent="submitForm"
            class="w-full max-w-full rounded-xl flex justify-between gap-4 bg-white">
            @csrf

            <!-- Image Upload -->
            <div class="min-w-20 sm:min-w-24 h-20 sm:h-24 aspect-square rounded-md overflow-hidden relative">
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('highlightManager', (initialData, userRole) => ({
                ...
                init() {this.$root.addEventListener('delete-highlight', (event) => {
