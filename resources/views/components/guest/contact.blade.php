@props(['classa' => null, 'classb' => null, 'notlp' => null, 'data' => null])
<div class=" w-full sticky bottom-0 py-2 px-4 sm:px-8 z-30 rounded-b-md">
    <div class="grid {{ $data->home_button === 'on' ? 'grid-cols-3' : 'grid-cols-2' }} gap-2 w-full max-w-[640px] mx-auto">
        @if ($data->home_button === 'on')    
            <a href="{{route('home')}}">
                <button
                    class=" text-base w-full flex flex-col items-center py-2 border rounded-md {{$classb ?? ''}} duration-300 relative">
        ...
        <a href="tel:{{ $notlp ?? '' }}">
            <button
                class=" text-base w-full flex flex-col items-center py-2 border rounded-md {{$classa ?? ''}} duration-300 relative">
                <div class=" w-5 aspect-square">
