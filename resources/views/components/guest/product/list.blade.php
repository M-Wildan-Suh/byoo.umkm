                ...
                @foreach ($data->productHighlight as $item)
                    <div style="background-color: {{ $template->product_main_color }};" class="w-full p-3 rounded-xl flex gap-2 text-white relative">
                        ...
                        <div class="w-full flex flex-col justify-between gap-1.5 sm:gap-2">
                            ...
                            <div class=" w-full flex gap-1.5 justify-between">
                                    ...
                                    @if ($role === "admin" || $role === "premium")
                                        <div class=" flex rounded-md overflow-hidden">
                                            ...
                                            <label 
                                                ...
                                                class="duration-300 rounded-md py-1 px-3 text-sm cursor-pointer border-2 border-white relative overflow-hidden">
                                                {{$data->order_title}}
                                                <div :class="checkedItems.some(data => data.id === {{ $item->id }}) ? 'bg-black opacity-50' : ''" class=" absolute inset-0 duration-300"></div>
                                            </label>
                                        </div>
                                    @endif
                                </div>
        ...
        <div 
            ...
            <div 
                ...
                <div class=" w-full flex justify-end">
                    <button onclick="document.getElementById('myForm') ? document.getElementById('myForm').submit() : console.error('Form tidak ditemukan!')" class=" py-1.5 px-3 flex items-center gap-2 text-sm rounded-md border border-white hover:bg-white/50 duration-300">
                        <div class=" w-4 h-4">
