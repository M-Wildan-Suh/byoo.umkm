            ...
            <div class="grid grid-cols-1 gap-3">
                @foreach ($data->productHighlight as $item)
                    <div style="background-color: {{ $template->product_main_color }};" class="w-full p-3 rounded-xl flex gap-2 text-white relative">
                        ...
                        <div class="w-full flex flex-col sm:justify-between gap-1.5 sm:gap-2">
                            <p class="line-clamp-1 sm:text-lg font-semibold">{{ $item->title }}</p>
                            <p class="line-clamp-2 text-sm sm:text-base">{{ $item->description }}</p>
                            <div class=" absolute sm:static flex justify-end right-2 -translate-y-2/3 sm:-translate-y-0 top-full">
                                @if ($role === "admin" || $role === "premium")
                                    <div class=" flex rounded-md bg-black overflow-hidden">
                                        <input 
                                            type="checkbox" 
                                            class="hidden" 
                                            name="order[]" 
        ...
        <div 
            ...
            <div 
                ...
                <div class=" w-full flex justify-end">
                    <button onclick="document.getElementById('myForm') ? document.getElementById('myForm').submit() : console.error('Form tidak ditemukan!')" class=" py-1.5 px-3 flex items-center gap-2 text-sm rounded-md hover:bg-white/50 duration-300">
                        <div class=" w-4 h-4">
